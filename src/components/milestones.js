  ...
  const columnsHTML = data.years
    .map((yearData, index) => {
      ...
      <div class="milestone-card bg-white p-4 rounded-lg border border-slate-200 shadow-sm flex items-start h-full">
        <span class="inline-block w-2 h-2 bg-ohm-red rounded-full mr-3 mt-1.5 shrink-0"></span>
    ...
    `
      ...
      <li class="relative flex flex-col gap-y-4">
        ...
        <div class="bg-ohm-red text-white font-bold p-3 text-center rounded-lg shadow-md relative z-10">
          ${yearData.label}
        </div>
